...
with st.sidebar:
    ...
    with st.expander("G(s) - Função de transferência", expanded=True):

        st.latex(r"G(s) = K \cdot \frac{N_G(s)}{D_G(s)}")

        txt_nG = st.text_input(
            "Numerador G(s)",
            value="1 2",
            help="Coeficientes do numerador em ordem decrescente de potência de s."
        )

        txt_dG = st.text_input(
            "Denominador G(s)",
            value="1 4 0",
            help="Coeficientes do denominador em ordem decrescente de potência de s."
        )

    with st.expander("H(s) - Realimentação", expanded=True):

        st.latex(r"H(s) = \frac{N_H(s)}{D_H(s)}")

        txt_nH = st.text_input(
            "Numerador H(s)",
            value="1",
            help="Coeficientes do numerador em ordem decrescente de potência de s."
        )

        txt_dH = st.text_input(
            "Denominador H(s)",
            value="1 1",
            help="Coeficientes do denominador em ordem decrescente de potência de s."
        )

    with st.expander("Ponto de teste", expanded=False):

        st.caption("Utilizado nos Passos 11 e 12.")
    ...
    with st.expander("Limites dos gráficos", expanded=False):

        usar_limites = st.checkbox(
        ...
        if usar_limites:
            ...
            # Valores padrão para evitar variáveis não definidas
            x_min_usr = None
            x_max_usr = None
            y_min_usr = None
            y_max_usr = None
...
# ================================================================
# VERIFICA SE O USUÁRIO JÁ SOLICITOU O CÁLCULO
# ================================================================
if not st.session_state.get("calcular_lgr", False):
    st.info("Configure o sistema na barra lateral e clique em **Calcular LGR**.")
    st.stop()
...
if any(x is None for x in [nG, dG, nH, dH]):
    st.error("Confira os coeficientes informados. Há algum valor inválido.")
    st.stop()
